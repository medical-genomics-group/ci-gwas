...
void marker_pheno_corr_dist(int argc, char *argv[])
{
    ...
    write_floats_to_binary(phen_corr.data(), phen_corr_mat_size, make_path(outdir, "pxp", ".corr"));
...
arguments:
    ...
    void block_diagonal_pc(int argc, char *argv[])
    {
        ...
        for (size_t bid = first_block; bid < blocks.size(); bid++)
        {
            ...
            // load block data
            std::vector<unsigned char> bedblock =
                read_block_from_bed(bfiles.bed(), block, dims, bim);
...
arguments:
    ...
    void scorr(int argc, char *argv[])
    {
        ...
        // write results
        write_floats_to_binary(
            corrs.data(), corr_mat_size, make_path(out_dir, chr_id, "_scorr.bin")
        );
